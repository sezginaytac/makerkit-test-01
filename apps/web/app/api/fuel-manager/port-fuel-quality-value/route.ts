...
// GET /api/fuel-manager/port-fuel-quality-value - Get all port fuel quality value data
export async function GET(request: NextRequest) {
  try {
    console.log('=== Port Fuel Quality Value GET API Debug ===');
    ...
    // Get all port fuel quality value data
    ...
    console.error('Error in GET /api/fuel-manager/port-fuel-quality-value:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

// POST /api/fuel-manager/port-fuel-quality-value - Create new port fuel quality value
export async function POST(request: NextRequest) {
  try {
    console.log('=== Port Fuel Quality Value POST API Debug ===');
    console.log('Request headers:', Object.fromEntries(request.headers.entries()));
    console.log('Request cookies:', request.cookies.getAll());
    ...
    // Create new port fuel quality value
    const { data: newPortFuelQualityValue, error } = await supabase
      .from('port_fuel_quality_value')
      .insert({
    ...
    if (error) {
      console.error('Error creating port fuel quality value:', error);
      return NextResponse.json({ 
        error: 'Failed to create port fuel quality value',
    ...
    return NextResponse.json({
      message: 'Port fuel quality value created successfully',
      portFuelQualityValue: newPortFuelQualityValue
    });
  } catch (error) {
    console.error('Error in POST /api/fuel-manager/port-fuel-quality-value:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create port fuel quality value' },
      { status: 500 }
    );
  }
}
