...
// GET /api/fuel-manager/ships/by-name/[name] - Get ship by name
export async function GET(
  request: NextRequest,
  { params }: { params: { name: string } }
) {
  try {
    console.log('=== Get Ship by Name API Debug ===');
    console.log('Ship name:', params.name);
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        return NextResponse.json(
          { error: `Ship not found with name: ${decodeURIComponent(params.name)}` },
          { status: 404 }
        );
      }
    ...
    console.error('Error in GET /api/fuel-manager/ships/by-name/[name]:', error);
