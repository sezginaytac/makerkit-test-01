...
export async function GET(request: NextRequest) {
  try {
    console.log('=== Ship Names API Debug ===');
    ...
    // Get all ship names for the user
    const shipNames = await fuelInventoryService.getShipNames(user.id);

    return NextResponse.json(shipNames);
  } catch (error) {
    console.error('Error fetching ship names:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch ship names' },
      { status: 500 }
    );
  }
}
